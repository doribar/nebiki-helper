...
create table if not exists public.area_count_records (
  ...
  actual_weekday_group text not null check (actual_weekday_group in ('月水', '火木', '金土日', '火木日', '金土', '三連休中日')),
...
-- 時刻別・三連休中日の新グループ名を保存できるよう、既存列の許可値だけを拡張します。
...
alter table public.area_count_records
  ...
  check (actual_weekday_group in ('月水', '火木', '金土日', '火木日', '金土', '三連休中日'));
