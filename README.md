...
- ステップURLは残しますが、エリア残数履歴はstep1〜step8・通常版で共通です。
...
- step1: 表示値引率で一律値引
- step2: 曜日・時刻を基準に、多い商品を+10%
- step3: 多い・少ない・どちらでもないの3分類
- step4: 多い商品のうち10個以上だけ、さらに+5%
- step5: 定番商品・夜によく売れる商品を-10%
- step6: 見た目が悪い商品・不人気商品を+10%
- step7: 今日の商品の減り方も判断に含める
- step8: 広告商品の当日の売れ方まで含める全解禁

ハッシュなしURLはstep8として動作します。
...
- アプリ版: `2026.7.19-1`。データ形式変更なし。Step1では祝前日の注意を表示せず、Step2〜8は従来表示を維持します。今回の変更に伴うSupabase SQL再実行は不要です（前版の曜日グループ制約更新が未実行の場合は一度実行してください）。
